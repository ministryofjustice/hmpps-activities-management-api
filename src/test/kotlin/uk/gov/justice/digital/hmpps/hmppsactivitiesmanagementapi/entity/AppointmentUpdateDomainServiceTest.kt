...
class AppointmentUpdateDomainServiceTest {
  ...
  inner class UpdateAppointments {
    ...
    fun `adds prisoners to appointment`() {
      ...
      verify(outboundEventsService, times(6)).sendAppointmentEvent(eq(OutboundEvent.APPOINTMENT_INSTANCE_CREATED), any(), eq("TEST"))
      verifyNoMoreInteractions(outboundEventsService)
    }
    ...
    fun `removes prisoners from appointment`() {
      ...
      verify(outboundEventsService, times(6)).sendAppointmentEvent(eq(OutboundEvent.APPOINTMENT_INSTANCE_DELETED), any(), eq("TEST"))
      verifyNoMoreInteractions(outboundEventsService)
    }
    ...
    fun `adds and removes prisoners on appointment`() {
      ...
      verify(outboundEventsService, times(6)).sendAppointmentEvent(eq(OutboundEvent.APPOINTMENT_INSTANCE_DELETED), any(), eq("TEST"))
      verify(outboundEventsService, times(9)).sendAppointmentEvent(eq(OutboundEvent.APPOINTMENT_INSTANCE_CREATED), any(), eq("TEST"))
      verifyNoMoreInteractions(outboundEventsService)
    }
