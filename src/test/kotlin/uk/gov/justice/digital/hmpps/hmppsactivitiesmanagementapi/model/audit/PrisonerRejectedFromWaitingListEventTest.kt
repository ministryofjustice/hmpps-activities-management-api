...
class PrisonerRejectedFromWaitingListEventTest : AuditableEventTestBase() {
  ...
  fun `returns correct type`() {
    ...
    assertThat(event.auditEventType).isEqualTo(AuditEventType.PRISONER_REJECTED_FROM_WAITING_LIST)
  }

  @Test
  fun `returns correct string representation`() {
    val event = createEvent()
    val expectedToString = "Prisoner AA12346 was rejected from the waiting list for activity 'Some Activity'(1). " +
  ...
  fun `returns the correct LocalAuditRecord representation`() {
    ...
    val expectedLocalAuditRecord = LocalAuditRecord(
      ...
      detailType = AuditEventType.PRISONER_REJECTED_FROM_WAITING_LIST,
      recordedTime = LocalDateTime.of(2023, 3, 22, 9, 0, 3),
      prisonCode = "PBI",
      prisonerNumber = "AA12346",
      activityId = 1,
      message = "Prisoner AA12346 was rejected from the waiting list for activity 'Some Activity'(1). " +
        "Event created on 2023-03-22 at 09:00:03 by Bob.",
    )
  ...
  private fun createEvent(): PrisonerRejectedFromWaitingListEvent {
    val createdAt = LocalDateTime.of(2023, 3, 22, 9, 0, 3)
    return PrisonerRejectedFromWaitingListEvent(
      1,
      "Some Activity",
      "PBI",
      "AA12346",
