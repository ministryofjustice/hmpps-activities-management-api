...
class ActivityCategoryTest {
  ...
  fun `converted to model`() {
    val expectedModel = ModelActivityCategory(1, "category code", "category name", "category description")
    assertThat(activityCategory().toModel()).isEqualTo(expectedModel)
  }

  @Test
  fun `List converted to model`() {
    val expectedModel = listOf(ModelActivityCategory(1, "category code", "category name", "category description"))
