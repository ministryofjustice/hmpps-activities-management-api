...
class AppointmentOccurrenceAllocationTest {
  ...
  fun `entity to model mapping`() {
    ...
    assertThat(appointmentSeriesEntity().occurrences().first().allocations().first().toModel()).isEqualTo(expectedModel)
  }

  @Test
  fun `entity list to model list mapping`() {
    val expectedModel = listOf(appointmentOccurrenceAllocationModel())
    assertThat(appointmentSeriesEntity().occurrences().first().allocations().toModel()).isEqualTo(expectedModel)
  }
}
