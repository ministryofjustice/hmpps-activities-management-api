...
class ManageAttendanceRecordsJobIntegrationTest : LocalStackTestBase() {
  ...
  fun `Attendance records are not created for where there are exclusions`() {
    ...
    assertThat(attendanceRepository.findAll().filter { it.scheduledInstance.scheduledInstanceId == 1L }).hasSize(2)
