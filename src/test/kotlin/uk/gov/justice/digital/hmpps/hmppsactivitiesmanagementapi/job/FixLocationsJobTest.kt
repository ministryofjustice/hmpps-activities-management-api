...
import uk.gov.justice.digital.hmpps.hmppsactivitiesmanagementapi.entity.JobType.FIX_APPOINTMENT_LOCATIONS
...
class FixLocationsJobTest : JobsTestBase() {
  ...
  private val fixAppointmentLocationsService: FixAppointmentLocationsService = mock()
  private val jobDefinitionCaptor = argumentCaptor<JobDefinition>()

  private val job = FixLocationsJob(
    fixActivitiesLocationsService,
    fixAppointmentSeriesLocationsService,
    fixAppointmentSetLocationsService,
    fixAppointmentLocationsService,
    safeJobRunner,
  )

  @Test
  fun `fix locations job triggered`() {
    mockJobs(FIX_ACTIVITY_LOCATIONS, FIX_APPOINTMENT_SERIES_LOCATIONS, FIX_APPOINTMENT_SET_LOCATIONS, FIX_APPOINTMENT_LOCATIONS)

    job.execute()

    verify(safeJobRunner, times(4)).runJob(jobDefinitionCaptor.capture())
    ...
    verify(fixAppointmentLocationsService).fixLocations()
  }
}
