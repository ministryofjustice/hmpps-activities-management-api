...
class PrisonerUnsuspendedFromActivityEventTest : AuditableEventTestBase() {
  ...
  fun `returns correct type`() {
    ...
    assertThat(event.auditEventType).isEqualTo(AuditEventType.PRISONER_UNSUSPENDED_FROM_ACTIVITY)
  }

  @Test
  fun `returns correct string representation`() {
    val event = createEvent()
    val expectedToString = "Prisoner AA12346 was unsuspended from activity 'Some Activity'(1) " +
  ...
  fun `returns the correct LocalAuditRecord representation`() {
    ...
    val expectedLocalAuditRecord = LocalAuditRecord(
      ...
      detailType = AuditEventType.PRISONER_UNSUSPENDED_FROM_ACTIVITY,
      recordedTime = LocalDateTime.of(2023, 3, 22, 9, 0, 3),
      prisonCode = "PBI",
      prisonerNumber = "AA12346",
      activityId = 1,
      activityScheduleId = 42,
      message = "Prisoner AA12346 was unsuspended from activity 'Some Activity'(1) " +
        "scheduled on 2023-03-23 between 09:00 and 10:00 (scheduleId = 42). Event created on 2023-03-22 at 09:00:03 by Bob.",
    )
