...
class ScheduledEventIntegrationTest : IntegrationTestBase() {
  ...
  inner class GetInternalLocationEvents {
    ...
    @Test
    @Sql("classpath:test_data/seed-activity-id-3.sql")
    @Sql("classpath:test_data/seed-appointments-1.sql")
    fun `get location events for single location with activities, appointments and visits - 200 success`() {
      val date = LocalDate.of(2022, 10, 1)

      val visit = PrisonApiPrisonerScheduleFixture.visitInstance(
        eventId = 8,
        locationId = activityLocation1.locationId,
        date = LocalDate.of(2022, 10, 1),
      )

      prisonApiMockServer.stubScheduledVisitsForLocation(prisonCode, activityLocation1.locationId, date, null, listOf(visit))
      ...
      with(result) {
        ...
        events hasSize 4
        events.filter { it.scheduledInstanceId == 1L && it.eventType == "ACTIVITY" } hasSize 2
        events.filter { it.eventType == "APPOINTMENT" } hasSize 1
        events.filter { it.eventType == "VISIT" } hasSize 1
      }
    }
    ...
    fun `get location events ignores activities with location id and on-wing is true`() {
      val dpsLocationId = activityDpsLocation1.id
      val date = LocalDate.of(2022, 10, 1)
      ...
      val result = webTestClient.getLocationEvents(prisonCode, dpsLocationId, date)!!
    ...
    fun `get location events includes events where future not required is true`() {
      ...
      val result = webTestClient.getLocationEvents(prisonCode, dpsLocationId, tomorrow)!!
    ...
    private fun WebTestClient.getLocationEvents(
      ...
      .expectStatus().isOk
      .expectHeader().contentType(MediaType.APPLICATION_JSON)
      .expectBody(InternalLocationEvents::class.java)
      .returnResult().responseBody
  }
