...
import uk.gov.justice.digital.hmpps.hmppsactivitiesmanagementapi.repository.refdata.CANCELLED_APPOINTMENT_CANCELLATION_REASON_ID
...
class AppointmentCreateDomainService(
  ...
  fun createAppointments(
    ...
    val cancellationReason = if (isCancelled) {
      appointmentCancellationReasonRepository.findOrThrowNotFound(
        CANCELLED_APPOINTMENT_CANCELLATION_REASON_ID,
      )
    } else {
      null
    }
