...
data class Attendance(
  ...
  /*
   ...
   1. It is an attendance required activity
   2. It has not been marked, and we are within 14 days of the session date.
   3. It has been marked (paid or unpaid) and today is the date of the session.
   4. It has been marked with an unpaid reason, and we are within 14 days of the session date.
   5. It has been marked with a paid reason, it was marked today, and we are within 14 days of the session date.

   */
  fun editable(): Boolean {
    return (
      this.scheduledInstance.attendanceRequired() && (this.status() == AttendanceStatus.WAITING &&
        this.scheduledInstance.sessionDate.isAfter(LocalDate.now().minusDays(14)) || this.status == AttendanceStatus.COMPLETED &&
        this.scheduledInstance.sessionDate.isEqual(LocalDate.now()) || this.status == AttendanceStatus.COMPLETED &&
