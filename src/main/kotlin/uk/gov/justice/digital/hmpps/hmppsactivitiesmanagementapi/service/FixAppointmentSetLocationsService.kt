...
@Service
class FixAppointmentSetLocationsService(
  ...
  fun fixLocations() {
    val ids = appointmentSetRepository.findNomisLocationsIds()

    log.info("Found ${ids.size} appointment set locations ids with null DPS Location UUIDS")
    ...
    ids.mapNotNull { internalLocationId ->
      ...
      transactionHandler.newSpringTransaction {
        appointmentSetRepository.updateLocationDetails(
          internalLocationId,
          dpsLocation.id,
        )
      }
    ...
    log.info("Finished running appointment set fix locations job")
  }
}
